...
public class UserServiceImpl implements UserService {
  ...
  public Page<Task> getTasks(
      ...
      int pageNo) {
    Page<Task> tasks =
        taskRepository.findTasksBySearchTermOrStatusOrPriorityOrderByDueDate(
            SecurityHelper.getLoggedInUser().getId(),
            searchTerm,
            status,
            priority,
            dueDateFrom,
            dueDateUntil,
            Pageable.ofSize(AppConstants.PAGE_SIZE).withPage(pageNo));
  ...
  public Task getTask(Long taskId) throws TaskException {
    Task task =
        taskRepository.findById(taskId).orElseThrow(() -> new TaskException("Task not found"));
