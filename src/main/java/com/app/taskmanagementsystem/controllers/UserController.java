...
public class UserController {
  ...
  public Response<Task> createTask(@RequestBody @Valid TaskDto taskDto) {
    return Response.<Task>builder()
        .data(userService.createTask(taskDto))
        .status(Response.Status.SUCCESS)
        .build();
  }

  @GetMapping("/")
  public PageResponse<List<Task>> getTasks(
    ...
    Page<Task> pageOfTasks =
        userService.getTasks(searchTerm, status, priority, dueDateFrom, dueDateUntil, pageNo);
    return PageResponse.<List<Task>>builder()
        .data(pageOfTasks.getContent())
  ...
  @PutMapping("/{taskId}")
  public Response<Task> updateTask(@PathVariable Long taskId, @RequestBody @Valid TaskDto taskDto)
      throws TaskException {
    return Response.<Task>builder()
        .data(userService.updateTask(taskId, taskDto))
        .status(Response.Status.SUCCESS)
        .build();
  }
  ...
  public Response<Task> deleteTask(@PathVariable Long taskId) throws TaskException {
    return Response.<Task>builder()
        .data(userService.deleteTask(taskId))
        .status(Response.Status.SUCCESS)
        .build();
  }
}
